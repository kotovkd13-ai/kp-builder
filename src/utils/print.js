import { formatMoney, formatDate, pluralSotrudnik, getKpNumber, buildContentLines } from './calc';
...
export function generateAndPrint(state) {
  const {
    client, inn, date, validity, manager, phone, email, maxUser,
    payment, discount, kv, instDates, rows, taskBlocks, scBlocks,
    totSum, kvAmt, discAmt, afterDisc,
  } = state;
  ...
  // Use pre-calculated values passed from Builder
  const _totSum    = totSum    !== undefined ? totSum    : rows.reduce((s, r) => s + (parseFloat(r.price)||0)*(parseInt(r.count)||0), 0);
  const _kvAmt     = kvAmt     !== undefined ? kvAmt     : Math.round(_totSum * kvPct / 100);
  const _discAmt   = discAmt   !== undefined ? discAmt   : Math.round((_totSum + _kvAmt) * discPct / 100);
  const _afterDisc = afterDisc !== undefined ? afterDisc : (_totSum + _kvAmt) - _discAmt;
  ...
  // ── Discount / KV rows ──
  let discRows = '';
  if (_kvAmt > 0 || _discAmt > 0) {
    if (_kvAmt > 0) {
      discRows += `<tr class="disc-row"><td colspan="6">КВ ${kvPct}%</td><td class="num">+${formatMoney(_kvAmt)}</td></tr>`;
    }
    if (_discAmt > 0) {
      discRows += `<tr class="disc-row"><td colspan="6">Скидка ${discPct}%</td><td class="num">&#8722;${formatMoney(_discAmt)}</td></tr>`;
    }
    discRows += `<tr class="disc-row bold"><td colspan="6"><strong>Итого</strong></td><td class="num"><strong>${formatMoney(_afterDisc)}</strong></td></tr>`;
  }

  // ── Installment ──
  let instHTML = '';
  if ((payment === '3' || payment === '4') && instDates && instDates.length) {
    const finalAmt = _afterDisc > 0 ? _afterDisc : _totSum;
    const cells = instDates.map((d, i) => {
      const pct = parseFloat(d.pct) || 0;
      const amt = Math.round(finalAmt * pct / 100);
      const dStr = d.date ? formatDate(d.date) : '—';
      return `<div class="inst-cell">
        <div class="ic-lbl">Платёж ${i + 1} &middot; ${pct}%</div>
        <div class="ic-date">${dStr}</div>
        <div class="ic-val">${formatMoney(amt)}</div>
      </div>`;
    }).join('');
    instHTML = `<div class="p-section avoid-break">
      <h2>График платежей</h2>
      <div class="inst-grid" style="grid-template-columns:repeat(${instDates.length},1fr)">${cells}</div>
    </div>`;
  }

  // ── Tasks ──
  const tasksHTML = taskBlocks && taskBlocks.length
    ? `<ul class="task-list">${taskBlocks.map(b =>
        `<li class="avoid-break"><span class="task-title">${esc(b.title || '—')}</span>${b.body ? ` <span class="task-body">${esc(b.body)}</span>` : ''}</li>`
      ).join('')}</ul>`
    : '<p style="color:#9ca3af;font-size:12px;">Задачи не указаны</p>';

  // ── Special conditions ──
  const scHTML = scBlocks && scBlocks.length
  ...
  if (rows.length > 1) {
    ...
    const bodyRows = COMPARE_OPTIONS.map(opt => {
      ...
      const cells = vals.map(v => `<td class="${!allSame && v.text !== '—' ? 'diff' : ''}">${v.green ? `<span style="color:#1a9e5c;font-weight:700">${v.text}</span>` : '<span style="color:#ccc">—</span>'}</td>`).join('');
      return `<tr><td class="rh">${opt.label}</td>${cells}</tr>`;
    }).join('');
    const priceCells = rows.map(r => `<td>${r.price ? formatMoney(parseFloat(r.price)) : '—'}</td>`).join('');
    const totalCells = rows.map(r => {
      const n = parseInt(r.count) || 0;
      const p = parseFloat(r.price) || 0;
      return `<td>${p > 0 ? formatMoney(p * n) : '—'}<br><small>${n} чел.</small></td>`;
    }).join('');
...
.cover-meta-item label{display:block;font-size:9px;font-weight:700;color:rgba(255,255,255,.38);text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px;}
  ...
  <table class="kp-tbl">
    ...
    <tfoot>
      <tr class="kp-foot avoid-break">
        ...
        <td></td>
        <td class="num">${formatMoney(_totSum)}</td>
      </tr>
      ${discRows}
    </tfoot>
  </table>
...
<div class="p-section">
  ...
  <div class="svc-grid">
    <div class="svc-item"><strong>Онлайн-консультации врачей</strong><p>Круглосуточно, без ограничений по количеству. 25+ специальностей.</p></div>
    <div class="svc-item"><strong>Амбулаторно-поликлиническая помощь</strong><p>Очные приёмы в клиниках-партнёрах по всей России.</p></div>
    <div class="svc-item"><strong>Лабораторная и инструментальная диагностика</strong><p>УЗИ, ЭКГ, КТ, МРТ, лабораторные исследования по направлению врача.</p></div>
    ...
    <div class="svc-item"><strong>Наблюдение хронических состояний</strong><p>Диспансерное наблюдение с заданным объёмом консультаций и исследований.</p></div>
    <div class="svc-item"><strong>Поддерживающее наблюдение при онкологии</strong><p>Сопровождение на этапе поддерживающей терапии.</p></div>
    ...
    <div class="svc-item"><strong>Чекап и профилактика</strong><p>Комплексный чекап, профилактические обследования, вакцинация.</p></div>
    <div class="svc-item"><strong>Стационарная помощь</strong><p>Экстренная и плановая госпитализация.</p></div>
    <div class="svc-item"><strong>Скорая и неотложная помощь</strong><p>Организация скорой помощи в пределах территории договора.</p></div>
...
<div class="p-section">
  ...
  <div class="step-list">
    ...
    <div class="step-item"><div class="step-num">3</div><div class="step-content"><strong>Медицинский советник помогает с записью</strong><p>Организует приём в клинике, лабораторные исследования, диагностику или госпитализацию по медицинским показаниям.</p></div></div>
  </div>
</div>

${cmpHTML}

${scBlocks && scBlocks.length ? `<div class="p-section sc-section" style="page-break-before:always"><h2>Специальные условия</h2>${scHTML}</div>` : ''}
